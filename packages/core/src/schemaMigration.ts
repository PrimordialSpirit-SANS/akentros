...
// DDL 方言:時間戳一律 TEXT 存 UTC ISO-8601(含毫秒),兩種方言共用相同的
// 欄位型別與字串比較慣例(布林以 INTEGER 1/0;JSON 以 TEXT);差異僅在
// 時間戳預設值與自增主鍵的語法:
// - SQLite:strftime('%Y-%m-%dT%H:%M:%fZ','now')、INTEGER PRIMARY KEY AUTOINCREMENT
// - PostgreSQL:to_char(now() AT TIME ZONE 'UTC', …)、BIGINT … AS IDENTITY
const NOW_DEFAULT_SQLITE = "TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))";
const NOW_DEFAULT_POSTGRES =
  "TEXT NOT NULL DEFAULT (to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'))";

function nowDefault(dialect: "postgres" | "sqlite"): string {
  return dialect === "postgres" ? NOW_DEFAULT_POSTGRES : NOW_DEFAULT_SQLITE;
}

// GENERATED BY DEFAULT(而非 ALWAYS)保留顯式插入 id 的能力,測試 fixtures
// 與部分 seed 流程依賴此。
const ID_PRIMARY_SQLITE = "INTEGER PRIMARY KEY AUTOINCREMENT";
const ID_PRIMARY_POSTGRES = "BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY";

function idPrimary(dialect: "postgres" | "sqlite"): string {
  return dialect === "postgres" ? ID_PRIMARY_POSTGRES : ID_PRIMARY_SQLITE;
}

function migrationTableSql(dialect: "postgres" | "sqlite"): string {
  return `
  CREATE TABLE IF NOT EXISTS akentros_ai_schema_migrations (
    version INTEGER PRIMARY KEY,
    name VARCHAR(120) NOT NULL,
    applied_at ${nowDefault(dialect)}
  )
`;
}

function canonicalTables(dialect: "postgres" | "sqlite"): string[] {
  const now = nowDefault(dialect);
  const id = idPrimary(dialect);
  return [
...
function buildAkentrosMigrations(dialect: "postgres" | "sqlite") {
  return [
    Object.freeze({
      version: 1,
      name: dialect === "postgres" ? "canonical-runtime-schema-postgres" : "canonical-runtime-schema-sqlite",
      statements: Object.freeze([...canonicalTables(dialect), ...CANONICAL_INDEXES]),
    }),
    Object.freeze({
      version: 2,
    ...
    Object.freeze({
      version: 3,
      name: "idempotency-replay-store",
      statements: Object.freeze([
        // 金鑰級冪等重放 opt-in:0 = 關閉(完成鍵重送回 409,既有語意),
        // > 0 = 成功回應落地該 TTL 秒,完成鍵重送直接重放原始回應。
        // canonical v1 不含此欄位,統一由此遷移加入,避免 fresh/upgrade
        // 兩條路徑的 ALTER 行為分歧。
        `ALTER TABLE ai_api_keys ADD COLUMN idempotency_replay_ttl_seconds INTEGER NOT NULL DEFAULT 0`,
        // 重放命中不執行推論,但會在 ai_requests 落一列 status='replayed' 的
        // 輕量紀錄(計費欄位全 0),讓管理面看得到重放事件;此欄指向被重放的
        // 原始請求。idempotency_key 該列存 NULL,避開 (api_key_id,
        // idempotency_key) 的部分唯一索引。
        `ALTER TABLE ai_requests ADD COLUMN replay_of_request_id VARCHAR(80) NULL`,
        ...
        expires_at TEXT NOT NULL,
        created_at ${nowDefault(dialect)}
      )`,
        `CREATE UNIQUE INDEX IF NOT EXISTS idx_ai_idempotency_replays_key
...
/** 指定方言的版本化遷移清單(結構與 AKENTROS_MIGRATIONS 相同)。 */
export function akentrosMigrations(dialect: "postgres" | "sqlite") {
  return buildAkentrosMigrations(dialect);
}

// 預設(SQLite)方言的遷移清單:既有契約測試釘死此常數的形狀與內容。
export const AKENTROS_MIGRATIONS = Object.freeze(buildAkentrosMigrations("sqlite"));
...
export async function migrateAkentrosSchema(query: any) {
  ...
  await query(migrationTableSql(dialect));
  for (const migration of buildAkentrosMigrations(dialect)) {
    const alreadyRecorded = await query(
      "SELECT version FROM akentros_ai_schema_migrations WHERE version = ?",
      [migration.version],
    );
