...
test("Hono exposes the complete Beacon developer route contract", () => {
...
test("developer key routes preserve authorization and secret handling guards", () => {
  ...
  assert.match(route, /isBeaconServiceRestricted/);
  assert.match(route, /Cache-Control['"], ['"]no-store/);
  assert.match(route, /beacon-key-management/);
  assert.match(route, /BEACON_API_KEY_PEPPER_INVALID/);
  assert.doesNotMatch(route, /console\.(?:log|error)\([^\n]*(?:req\.body|api_key|secret)/);
  assert.match(keys, /withBeaconTransaction/);
  assert.match(keys, /activeCount >= BEACON_MAX_ACTIVE_KEYS/);
  assert.match(keys, /BEACON_MAX_ACTIVE_KEYS/);
...
test("account inference uses a private session credential behind login auth", () => {
  ...
  const keys = read("apps/gateway/src/utils/aiApiKeys.ts");
  const stream = read("apps/console/src/lib/beacon/api/beaconChatStream.ts");
  const page = read("apps/console/src/pages/PlaygroundPage.tsx");

  assert.match(route, /authenticateToken/);
  assert.match(route, /post\(['"]\/chat\/completions['"]/);
  assert.match(route, /ensureBeaconSessionCredential/);
  assert.match(route, /handleBeaconChatCompletions/);
  assert.match(publicRoute, /authenticateBeaconKey/);
  assert.match(publicRoute, /handleBeaconChatCompletions/);
  assert.match(keys, /environment <> 'session'/);
  assert.match(keys, /public-key authentication excludes/);
  assert.match(stream, /getBeaconAccountChatCompletionsPath/);
  assert.match(stream, /authMode === ['"]account['"] \? apiFetch : fetch/);
  assert.match(page, /sk-beacon-live/);
});
...
test("Standalone gateway pins the Node entrypoint and workspace scripts", () => {
  ...
  assert.match(entrypoint, /installNodeBeaconDbAdapter/);
  // 維護迴圈本體由共用模組提供;進入點必須呼叫它,不得自帶一份。
  assert.match(entrypoint, /runBeaconMaintenance/);
  assert.match(maintenance, /cleanupBeaconRateLimitBuckets/);
  assert.match(maintenance, /reconcileStaleBeaconReservations/);
  assert.match(maintenance, /resolveQuarantinedBeaconReservations/);
  assert.match(scripts.migrate, /@beacon\/gateway/);
...
test("Cloudflare Workers deployment pins the Durable Object topology", () => {
  ...
  assert.match(worker, /export \{ BeaconGateway \}/);
  assert.match(worker, /idFromName\(/);
  assert.match(worker, /MAINTENANCE_PATH/);
  assert.match(wrangler, /"class_name": "BeaconGateway"/);
  assert.match(wrangler, /new_sqlite_classes/);
  assert.match(wrangler, /"crons"/);
  assert.match(wrangler, /nodejs_compat/);
  // DO 建構子安裝 storage.sql adapter;首次請求前完成遷移與管理員種子。
  assert.match(doClass, /installBeaconDbAdapter\(createDoBeaconDbAdapter/);
  assert.match(doClass, /ensureBeaconSchemaReady/);
  assert.match(doClass, /seedBeaconAdminFromEnv/);
  assert.match(doClass, /runBeaconMaintenance/);
...
test("Hono exposes the public Beacon inference routes", () => {
