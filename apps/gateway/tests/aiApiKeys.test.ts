...
async function insertKeyWithExpiry(userId: string, secretBody: string, expiresAt: string | null) {
  const secret = `sk-akentros-live_${secretBody}`;
  ...
  await dbQuery(
    ...
    VALUES (?, 'expiry regression', 'live', 'sk-akentros-live_', ?, ?, ?)
  `,
...
test("authentication rejects a key that expired earlier on the current UTC day", async () => {
  ...
  assert.equal(await authenticateAkentrosApiKey(env, expiredSecret), null);
});
...
test("authentication accepts an unexpired key and returns no phantom user fields", async () => {
  ...
  const key = await authenticateAkentrosApiKey(env, activeSecret);
...
test("created keys store ISO-8601 expiry that round-trips through authentication", async () => {
  ...
  const created = await createAkentrosApiKey(env, userId, {
    name: "ttl round-trip",
  ...
  assert.ok(await authenticateAkentrosApiKey(env, created.secret), "created key must authenticate");

  await assert.rejects(
    () =>
      createAkentrosApiKey(env, userId, {
        name: "short ttl",
