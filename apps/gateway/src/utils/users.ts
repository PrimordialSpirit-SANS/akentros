...
import { dbGet, dbQuery, runtimeDialect } from "./db.ts";
...
// 內建帳號系統的 users DDL:欄位型別與 SQLite/PostgreSQL 兩種方言共用
// (時間戳 TEXT 存 UTC ISO-8601、布林 INTEGER 1/0),差異僅在自增主鍵與
// 時間戳預設值語法,由 runtimeDialect 選取。
function usersTableDdl(dialect: "postgres" | "sqlite"): string {
  const id =
    dialect === "postgres"
...
export async function ensureUsersSchema(env: AkentrosRuntimeEnv): Promise<void> {
  await dbQuery(env, usersTableDdl(runtimeDialect(env)));
}
