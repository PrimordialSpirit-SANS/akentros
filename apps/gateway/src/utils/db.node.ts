...
// scripts 共用。此模組只應由 Node 進入點透過 installNodeAkentrosDbAdapter()
...
// 若以 cwd 解析 AKENTROS_DB_PATH=./akentros.db,兩者會開到不同的檔案——
 ...
 * 匯出供測試釘死 cwd 不變式。
 */
export function resolveDatabasePath(env: AkentrosRuntimeEnv): string {
  const configured = String(env?.AKENTROS_DB_PATH || env?.DATABASE_URL || "").trim();
  ...
  if (!candidate || candidate.startsWith("sqlite://") || candidate.startsWith("file:")) {
    candidate = decodeURIComponent(candidate.replace(/^(?:sqlite:\/\/|file:)/, "")) || "akentros.db";
  }
...
function databaseFor(env: AkentrosRuntimeEnv): DatabaseSync {
...
export async function dbGet(env: AkentrosRuntimeEnv, sql: string, params: any[] = []): Promise<any | null> {
...
export function createAkentrosQuery(env: AkentrosRuntimeEnv) {
