// 平台整合接縫:console 邏輯層(lib/beacon)所有 API 呼叫的底層。
// 原始平台在此提供 JWT 會話、CSRF 與外部 API base;本獨立版以同源 cookie session
// 加上環境變數設定重現相同契約。離線示範模式由 demoApi.ts 以 fetch 攔截實現。

const API_BASE = (import.meta.env.VITE_BEACON_API_BASE ?? "/api").replace(/\/+$/, "");
...
export function getExternalDeveloperApiBase(): string {
  const configured = import.meta.env.VITE_BEACON_PUBLIC_API_BASE;
