...
本專案的 Beacon 子系統採乾淨重建:以可驗證的 API、資料、計費與 runtime 契約為基線,不繼承任何未經驗證的舊 AI 殘餘(未載入的價格 JSON、來源不明的圖示等)。

Beacon 由單一 Worker/Hono 後端提供行為,並以 Beacon points 計費。外部 provider 呼叫、點數帳本與多 instance credential 選擇也都要求可稽核、可恢復且不洩漏秘密。
...
- 管理面只使用內建帳號系統的登入 cookie(beacon_token)、CSRF 與即時帳戶狀態檢查;推理面只接受 `sk-beacon-*` Key,兩者不可互換。
...
- `apps/console/public/data/beacon/*` 只供介面展示,不得參與模型驗證、routing、provider 選擇或計費。
...
- Beacon Key secret 只在建立或輪替時回傳一次。資料庫只保存遮罩欄位與使用 pepper 的 HMAC digest。
- provider secret 只存在各 runtime secret;資料庫僅保存 opaque credential ID 與健康／lease 狀態。
- prompt、completion、完整 Beacon Key 與 provider token 不寫入資料庫、log 或 error response。
...
`/api/ai/*` 由 `BEACON_ENABLED` 控制,fail-closed:未明確設為 `true` 時整個入口不存在(404)。啟用前至少應驗收:點數不足時 provider attempt 為零、無負餘額或雙扣、Node／Worker 契約一致、SSE 首個 data chunk 後不 fallback、秘密與內容不落地,以及控制台 deep link／無障礙驗收通過。
